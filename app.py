...
with tab1:
    ...
    # ── show logging 貼り付け取り込み（目立つ位置に配置） ──
    with st.expander("📋 show logging を貼り付けて解析（クリックで開く）", expanded=False):
        st.caption("機器の `show logging` / `show logging syslog` 出力を丸ごと貼り付け → 解析して取り込み")
        _slt_col1, _slt_col2 = st.columns([3, 1])
        with _slt_col1:
            _sl_text_main = st.text_area(
                "show logging 出力", height=180, key="show_log_text_main",
                placeholder="*Jul  3 10:00:01.123: %LINK-3-UPDOWN: Interface Gi1/0/1, changed state to down\n"
        ...
        with _slt_col2:
            _sl_src_main = st.text_input("送信元IP/ホスト", value="pasted-device",
                                         key="show_log_src_main")
            _sl_go = st.button("🔍 解析して取り込み", use_container_width=True,
                               key="show_log_ingest_main", type="primary")
        if _sl_go:
            if _sl_text_main.strip():
                _r = _ingest_show_logging(_sl_text_main, _sl_src_main.strip() or "pasted-device")
                if _r["total"]:
                    _vb = " / ".join(f"{k}:{v}" for k, v in _r["by_vendor"].items())
                    st.success(f"✅ {_r['total']}件取り込みました（除外 {_r['skipped']} 行）　内訳: {_vb}")
                    st.info("下のログ一覧に反映されます。ページ下部の「📊 ログ一括 AI 分析」でまとめて評価できます。")
                else:
