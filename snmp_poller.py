...
# UCD-SNMP-MIB（net-snmp系。Linuxベースの機器＝富士通IPCOM等で使えることがある）
_UCD_MEM_TOTAL_OID = "1.3.6.1.4.1.2021.4.5.0"   # memTotalReal (kB)
_UCD_MEM_AVAIL_OID = "1.3.6.1.4.1.2021.4.6.0"   # memAvailReal (kB)
...
VENDOR_MIBS = {
    ...
    "Fujitsu Si-R/IPCOM": {
        # 富士通のIANA企業番号(211)配下。Si-R/IPCOMともプロプライエタリMIBの
        # OIDは未確認のため個別センサーは定義せず、ベンダー判別のみ行う。
        # CPU/メモリは標準MIB(HOST-RESOURCES-MIB/UCD-SNMP-MIB)への
        # 汎用フォールバックで取得する（poll_device内の_poll_hr_*参照）。
        "enterprise": "1.3.6.1.4.1.211",
        "keywords": ["si-r", "fujitsu si-r", "ipcom", "sr-s"],
        "oids": {},
    },
}
...
def _poll_hr_memory_pct(ip: str, community: str, port: int, version: str) -> float | None:
    """
    汎用フォールバックでメモリ使用率を返す（Cisco専用メモリが取れない機器向け）。
    1) HOST-RESOURCES-MIB hrStorageTable（物理メモリの行を検出）
    2) UCD-SNMP-MIB memTotalReal/memAvailReal（net-snmp系。富士通IPCOM等のLinuxベース機器向け）
    """
    try:
    ...
    try:
        total = snmp_get(ip, community, _UCD_MEM_TOTAL_OID, port, version)
        avail = snmp_get(ip, community, _UCD_MEM_AVAIL_OID, port, version)
        if total is not None and avail is not None:
            total_f, avail_f = float(total), float(avail)
            if total_f > 0:
                return round((total_f - avail_f) / total_f * 100, 1)
    except Exception:
        pass
